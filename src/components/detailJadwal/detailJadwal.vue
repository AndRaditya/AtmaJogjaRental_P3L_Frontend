	...
	<v-main class="list">
					...
					<v-card-text>
						<v-container>
							...
							<v-select
								...
								required
								filled
								label="Masukkan Pegawai"
								item-value="id_pegawai"
							>
								// Don't forget your props
								<template slot="selection" slot-scope="data">
									<!-- HTML that describe how select should render selected items -->
									{{ data.item.id_pegawai }} -
									{{ data.item.nama_pegawai }}
								</template>
							...
							<v-select
								:items="jadwals"
								v-model="form.id_jadwal_increment"
								required
								filled
								label="Masukkan ID Jadwal"
								item-value="id_jadwal_increment"
								item-text="id_jadwal"
							>
							</v-select>

							<v-text-field
								v-model="form.keterangan_jadwal"
								required
								filled
								label="Masukkan Keterangan Jadwal"
							>
							</v-text-field>
						</v-container>
					</v-card-text>
		...
		<v-dialog v-model="dialogConfirm" persistent width="400px">
			<v-card>
				...
				<v-card-text>
					Anda Yakin ingin menghapus detail jadwal ini?
				</v-card-text>
				<v-card-action>
					<v-spacer></v-spacer>
					<v-btn
...
export default {
	...
	methods: {
		...
		save() {
			{
				...
				this.detail_jadwal.append(
					"id_jadwal_increment",
					this.form.id_jadwal_increment
				);
				this.detail_jadwal.append(
					"keterangan_jadwal",
					this.form.keterangan_jadwal
				);
		...
		readDataPegawai() {
			var url = this.$api + "/cariPegawai_Shift";
			this.$http
				.get(url, {
					// headers: {
					//     Authorization: "Bearer " + localStorage.getItem("token"),
		...
		update() {
			...
			data.append("id_jadwal_increment", this.form.id_jadwal_increment);
			data.append("keterangan_jadwal", this.form.keterangan_jadwal);
