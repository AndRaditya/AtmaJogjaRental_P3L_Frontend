<template>
	<v-main class="list">
		...
		<!-- <v-card>
            ...
            <v-data-table :headers="headers" :items="datas" :search="search" :single-expand="singleExpand"
                :expanded.sync="expanded" item-key="task" show-expand class="elevation-1">
                <template v-slot:[`item.actions`]="{ item }">
                    <v-icon small class="mx-2" @click="editTransaksi(item)" color="red"> mdi-pencil </v-icon>
                    <v-icon small @click="deleteTransaksi(item)" color="success"> mdi-delete </v-icon>
                </template>
            </v-data-table>
        </v-card> -->
					...
					<v-card-text>
						<v-container>
							<v-text-field
								...
								required
								filled
								label="Masukkan Tanggal Transaksi"
								type="datetime-local"
							>
							</v-text-field>
							<!-- 
							...
							<v-select
								...
								clearable
								:items="customers"
								v-model="form.id_customer_increment"
								required
								filled
								label="Masukkan Nama Customer"
								item-value="id_customer_increment"
							>
								// Don't forget your props
								<template slot="selection" slot-scope="data">
									<!-- HTML that describe how select should render selected items -->
									{{ data.item.id_customer }} -
							...
							>
								...
								<template slot="selection" slot-scope="data">
									<!-- HTML that describe how select should render selected items -->
									{{ data.item.kode_promo }} -
									{{ data.item.diskon }}% -
									{{ data.item.status_promo }}
								</template>
								<template slot="item" slot-scope="data">
									<!-- HTML that describe how select should render items when the select is open -->
									{{ data.item.kode_promo }} -
							...
							<v-select
								...
								required
								filled
								label="Masukkan Nama Pegawai yang melayani"
...
export default {
	...
	data() {
		return {
			...
			deleteId: '',
			editId: '',
			pegawaiId: localStorage.getItem('id_pegawai'),
		};
	},
	...
	methods: {
		...
		save() {
			{
				...
				this.transaksi.append(
					...
					this.form.id_customer_increment
				);
				this.transaksi.append(
					'metode_pembayaran',
		...
		readDataPegawai() {
			var url = this.$api + '/pegawai/' + this.pegawaiId;
			this.$http
				.get(url, {
					// headers: {
					//     Authorization: "Bearer " + localStorage.getItem("token"),
		...
		update() {
			...
			var url = this.$api + '/updateFromPegawai/' + this.editId;
			this.load = true;
			this.$http
				.post(url, data, {
					// headers: {
